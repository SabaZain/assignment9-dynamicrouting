        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-6 mx-6'>
          ...
          {todos.map((todo, index) => (
            <div key={index} className='border-slate-600 border rounded-md  bg-slate-200 hover:bg-purple-300 p-4 text-center'>
              ...
              <p className='font-semibold'>Price: ${todo.price}</p>
              <div className='text-center'>
                <Link href={`/Client-Side/ProductDetail/${todo.id}`}>
              <button className='bg-black hover:bg-white text-white hover:text-black hover:font-bold p-2 rounded-md mt-3 mb-1'>See More</button>
              </Link>
              </div>
